...
custom_retry_strategy=oci.retry.RetryStrategyBuilder(
        ...
        retry_base_sleep_time_seconds=5,
        ).get_retry_strategy()
...
def check_region_connectivity(region, config, signer, custom_retry_strategy):
    ...
    try:
        ...
        # Validate the connecivity fetching Namespace name
        storage=oci.object_storage.ObjectStorageClient(config=config, signer=signer)
        storage.get_namespace(retry_strategy=custom_retry_strategy).data
...
def set_user_compartment(identity_client, args_su, args_comp, tenancy_id):
    ...
    if args_comp:
        valid_compartment = validate_compartment(identity_client, args_comp)
    ...
    while True:
        ...
        if user_input in {"Y", "YES"}:
            while True:
                user_compartment = input(yellow("\nDo you want to target a specific compartment OCID? [Y]es, [N]o: ")).strip().upper()

                if user_compartment not in {"Y", "YES", "N", "NO"}:
                    print(red("\nInvalid input. Please enter 'Y' or 'N'\n"))
                    continue

                if user_compartment in {"N", "NO"}:
                    return tenancy_id

                # Validate compartment OCID
                compartment_pattern = r"^ocid1\.(compartment|tenancy)\.oc(1|\d{2,3})\.\.[a-zA-Z0-9]{30,100}$"
                while True:
                    user_compartment = input(yellow("\nEnter a compartment OCID: ")).strip().lower()
                    if re.match(compartment_pattern, user_compartment):
                        valid_compartment = validate_compartment(identity_client, user_compartment)
                        if valid_compartment:
                            return valid_compartment
                    else:
                        print(red("\nInvalid OCID format. Please try again.\n"))
